...
import { PlusCircle, AlertCircle, TrendingUp, Package, ShoppingBag, Loader2 } from 'lucide-react'
...
function DashboardContent() {
  ...
  const loadDashboardData = useCallback(async () => {
    try {
      ...
      // 🔒 BUSCA PARALELA ULTRA-OTIMIZADA
      const [productsRes, salesRes, installmentsRes] = await Promise.all([
        supabase.from('products').select('id, cost_price, price, stock_quantity').eq('user_id', user.id),
        supabase.from('sales').select('id, sale_price, quantity, cost_price, product_id').eq('user_id', user.id),
      ...
      if (products) {
        ...
        if (sales) {
          monthlyRevenue = sales.reduce((acc, s) => acc + (Number(s.sale_price) * (Number(s.quantity) || 1)), 0)
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 gap-6">
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Link href="/admin/sales" className="bg-brand-primary p-8 rounded-[40px] text-white shadow-lg hover:scale-[1.02] transition-all duration-300 transform-gpu">
            ...
            <Link href="/admin/sales" className="bg-amber-500 p-8 rounded-[40px] text-white shadow-lg hover:scale-[1.02] transition-all duration-300 transform-gpu">
              <div className="flex justify-between items-start mb-4">
